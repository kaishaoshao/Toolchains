...
编写一个程序，使其能够**控制** 另一个进程，无论是启动一个新进程还是附加到一个已有的进程上 。这是所有调试功能的基础。

#### 第一部分：进程交互的核心机制（理论）

在写代码之前，本章首先介绍了 Linux 系统中用于实现这一目标的核心工具。

1. **`fork` 和 `exec`：Linux 创建新进程的方式**

   * `fork` ：这个系统调用会创建一个当前进程的几乎一模一样的副本 。原来的进程被称为“父进程”，新创建的副本被称为“子进程” 。它们唯一的区别是
     `fork()` 的返回值：在父进程中，它返回子进程的唯一标识符（PID）；在子进程中，它返回 0 。
   * **`exec`(执行)** **：子进程被创建后，通常会调用 **`exec` 家族的系统调用之一。这个调用会用一个全新的程序来**替换**子进程当前的程序内容 。
   * **对调试器的意义** **：这个“先 **`fork` 再 `exec`”的模式，正是我们的调试器用来**启动并调试一个新程序**的方法 。调试器是父进程，它

   `fork` 出一个子进程，然后子进程通过 `exec` 变身为我们想要调试的目标程序。
2. **`ptrace`：调试器的“瑞士军刀”**

   * **核心概念** **：**`ptrace`是 Linux 提供的**主要调试接口** ^^。它是一个功能极其丰富的系统调用，虽然 API 设计有些老旧，但几乎所有调试操作都依赖它 。
   * **函数原型** **：**`long ptrace(enum __ptrace_request request, pid_t pid, void *addr, void *data)` 。
   * `request`：你想要执行的操作，比如附加进程、读取寄存器等 。
   * `pid 3`：目标进程的 ID 。
   * `addr` 和 `data`：根据 `request` 的不同，用途也不同 。
   * **关键操作示例** ：
   * `PTRACE_ATTACH`：附加到一个已经存在的进程。
   * `PTRACE_TRACEME`：由子进程调用，表示“允许我的父进程调试我” 。
   * `PTRACE_CONT`：让一个已暂停的进程继续执行 (Continue)。
   * **重要提示** **：书中反复强调，** **必须检查 `ptrace` 的返回值** 。如果返回 -1，就表示出错了，不检查错误会导致极难发现的 bug 。

#### 第二部分：动手实践：实现附加与启动（代码）

这部分是本章的核心，详细描述了如何编写代码来实现两种调试模式。

1. **启动并调试新进程 (`kdb <程序名>`)**

   * **调试器（父进程）**：调用 `fork()` 创建子进程 。
   * **子进程** ：

   1. 立刻调用
      `ptrace(PTRACE_TRACEME, ...)`，请求被追踪。
   2. 然后调用
      `execlp(...)`，将自己替换成目标程序（例如 `yes`）。

   * **操作系统** ：在 `exec` 调用成功后，由于子进程已被标记为“被追踪”，操作系统会自动暂停子进程，并向父进程发送一个信号 。
   * **调试器（父进程）** ：调用 `waitpid()` 等待这个信号，确认子进程已准备就绪并已暂停 。
2. **附加到现有进程 (`kdb -p <PID>`)**

   * **调试器** **：直接调用 `ptrace(PTRACE_ATTACH, pid, ...)`。
   * **操作系统** **：向 PID 对应的目标进程发送一个 `SIGSTOP` 信号，强制其暂停 。
   * **调试器** **：同样调用 `waitpid()` 来等待目标进程完全暂停 。

#### 第三部分：构建用户界面和重构（软件工程实践）

光有核心功能还不够，还需要让用户能够方便地使用，并让代码结构更清晰。

1. **添加命令行界面**
   * 使用
     **GNU Readline** 库来创建一个交互式的命令行提示符 (`kdb>`)，它支持历史记录、自动补全等功能，提升用户体验 。
   * 程序进入一个主循环，不断读取用户输入的命令并处理 。
2. **实现第一个命令：`continue`**
   * 当用户输入
     `continue`（或缩写 `c`）时，调试器会调用 `ptrace(PTRACE_CONT, ...)` 来让暂停的进程恢复运行。
   * 恢复后，调试器会立刻再次调用
     `waitpid()`，进入下一次等待状态，直到目标进程下一次停止（比如遇到下一个断点、程序结束或崩溃）。
3. **重构为库 (`libkdb`)**
   * **动机** ：将核心的调试逻辑（所有与 `ptrace` 相关的代码）从主程序 `sdb` 中分离出来，封装成一个独立的库 `libkdb`。
   * **好处** ：
   * **可测试性** ：可以为 `libsdb` 编写独立的自动化测试，确保逻辑正确 。
   * **可重用性** ：其他人或程序可以直接使用这个库来实现自己的调试工具 。
   * **实现** ：
   * 创建一 `kdb::process` 类来封装一个被调试的进程 。所有 `ptrace` 调用都隐藏在这个类的方法（如 `launch`, `attach`, `resume`）内部 。
   * **错误处理** **：库不应该直接打印错误并退出程序，而是通过抛出异常 (**`kdb::error`) 的方式来报告错误，让调用者决定如何处理 。
   * **资源管理 (RAII)** **：**`sdb::process` 类的析构函数 (`~process()`) 会确保在调试器退出时，正确地与目标进程分离或将其终止，避免留下“僵尸进程” 。
4. **解析进程状态 (`stop_reason`)**
   * `waitpid()`返回的状态信息是一个需要解码的整数 。
   * 本章创建了一个 `sdb::stop_reason` 结构体，专门用来解析这个状态值 。它使用系统宏来判断进程停止的原因：
     * `WIFEXITED`：是否正常退出 。
     * `WIFSIGNALED`：是否被信号终止 。
     * `WIFSTOPPED`：是否被信号暂停 。
   * 这使得代码逻辑更清晰，我们能明确知道目标进程当前处于什么状态。
